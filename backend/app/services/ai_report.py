...
def _build_chat_system_prompt(user: User, db: Session, joint: bool = False) -> str:
    ...
    ctx_lines = [
        ...
        "You have inspect_current_finances (live accounts/budgets/goals), query_transactions (the real ledger),",
        "and project_finances (the real forecast engine). Use them whenever the question needs current detail",
        "instead of assuming the static summary is complete or relying on an older chat turn.",
        ...
        "FINANCIAL PLANS: when the user asks for a spending/saving strategy, income-routing plan, category targets,",
        "or a what-if lifestyle plan, inspect the current finances and run projections as needed. Include these exact",
        "sections so MUNI can save and visualize the answer: '## Recommended Monthly Plan' followed by a markdown",
        "table with columns Category, Monthly target, Funded by; '## Savings Allocation' followed by a markdown table",
        "with columns Account or goal, Monthly amount, Funded by; then '## Tradeoffs and assumptions'. Make every",
        "target a concrete monthly dollar amount. Ensure recommended spending plus savings does not exceed the stated",
        "monthly income; show the balance check. Treat the result as a draft recommendation, never as an applied change.",
        "",
